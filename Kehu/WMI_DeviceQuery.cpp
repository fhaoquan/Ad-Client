...
INT WMI_DeviceQuery( INT iQueryType, T_DEVICE_PROPERTY *properties, INT iSize )
{
    ...
    if( FAILED(hres) )
    {
		//MessageBox(0,L"初始化com",0,0);
        return -2;
    }

    // 设置COM的安全认证级别
	hres = CoInitializeSecurity( NULL, -1, NULL, NULL, RPC_C_AUTHN_LEVEL_DEFAULT, RPC_C_IMP_LEVEL_IMPERSONATE, NULL,EOAC_NONE,
   NULL);
	if( FAILED(hres) )
    {
		//MessageBox(0,L"设置COM的安全认证级别",0,0);
    ...
    if( FAILED(hres) )
    {
		//MessageBox(0,L"获得WMI连接COM接口",0,0);
		CoUninitialize();
        return -2;
    }
    ...
    if( FAILED(hres) )
    {
		pLoc->Release(); 
        CoUninitialize();
        //MessageBox(0,L"通过连接接口连接WMI的内核对象名ROOT\\CIMV2",0,0);
	...
	if( FAILED(hres) )
    {
        ...
        pLoc->Release();     
        CoUninitialize();
	    //MessageBox(0,L" 设置请求代理的安全级别",0,0);
    ...
    while( pEnumerator )
    {
		...
		{	// 获取属性值
			...
			//USES_CONVERSION;
			StringCchCopy( properties[iTotal].szProperty, PROPERTY_MAX_LEN, vtProperty.bstrVal );
